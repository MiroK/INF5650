...
SOLVERS ARE TAKEN FROM THE FENICS BOOK.
...
def grpc_driven_cavity(v_order, n_cells, stabilization):
  '''Parameter n_cells determins mesh size, v_order determines order of
  polynomials for velocity space.'''
  ...
  # add stabilization for equal order
  if stabilization and (v_order == 1):
    beta  = Constant(0.1)
    d2 = beta*h*h
    Rp = Rp + d2*k*inner(grad(q), grad(P))*dx

  # Assemble preconditioners, linearized navier-stokes operator
  ax  = inner(v, u)*dx + 0.5*k*inner(v, (dot(grad(u), u0)))*dx \
      + 0.5*k*2*nu*inner(sym(grad(v)), sym(grad(u)))*dx \
      - 0.5*k*nu*inner(v, dot(n, grad(u)))*ds

  ay1 = k**2*(inner(grad(q), grad(p)))*dx     # pressure space stiffness matrix
  ay2 = k**2*((1.0/(nu*k)) * q*p)*dx          # pressure space mass matrix
...
if __name__ == '__main__':
  u, phi = grpc_driven_cavity(v_order=1, n_cells=32, stabilization=True)
  ...
  # P1 not stab ~37s
  # P1 stav ~21
  # With this problem we get convergence with P1-P1 without
  # stabilization, but not that despite the problem being smaller
  # than P2-P1 it requires same time to compute. This points to
  # more time spent in the internal loop to get the convergence 
  # of the saddle point problem. If we add stabilization, there
  # is a clear speed up.
